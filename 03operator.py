...
# 관계식 : 관계연산자(대소, 순서)를 이용한 수식
print(10 > 3, 10 < 3)

# 논리식 : 논리연산자(논리 합/곱/부정)를 이용한 수식
# 또한, 둘 이상의 논리식이나 관계식으로 구성된 수식
# 논리식의 경우, 수식의 구성에 따라 단축식 평가가 short-circuit 가 가능
print((5 == 3) and (5 > 3))
print((5 != 3) or (5 > 3))
...
# and 연산시 첫번째 수식이 F 이면 단축식평가 적용
print(3 > 5 and 2 < 3)

# or 연산시 첫번째 수식이 T 이면 단축식평가 적용
print(2 < 3 or 3 < 5)

# 복헙 대입연산자 : 대입연산자와 산술연산자를 결합한 수식
# 보통 수식을 간단히 작성시 사용 - 축약표현
# 변수 = 변수 + 수식 => 변수 += 수식

# 연산자 우선순위
# 괄호내 연산자 -> 딘힝연산자 -> 산술연산자 -> 관계연산자 -> 논리연산자
