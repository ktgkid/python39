...
# 컴파일 오류 : 주로 문법적 오류를 의미 - 개발자가 처리 가능
# 논리 오류 : 실행 가능, 예상결과와 다른 결과가 나오는 오류 - 개발자가 처리 가능
# 시스템 오류 : 메모리 초과, 서버접속 불가등등의 오류 - 처리 불가
# 실행 오류 : 프로그램 실행 중 발생하는 예외적인 오류
# (ex 어떤 값을 0으로 나누는 경우, 리스트의 인덱스를 잘못 호출 하는 경우 등등
# 잘못된 코드 사용으로 인한 오류)
...
try:
    print('- 프로그램 시작 -')
    print(10/0)               # 오류가 발생하면 except 로 점프
...
idx = int(input('nums 에서 사용할 값의 index 는?'))
